...
export default {
    ...
    methods: {
        ...
        fetchItems() {
            ...
            database.collection('inputs').where("userid", "==", this.currID).get().then(querySnapShot => { //access data for calories OUT
            ...
            database.collection('caloriesIn').where("userid", "==", this.currID).get().then(querySnapShot => { //access data for calories IN
                querySnapShot.forEach(doc => {
                    for(let i = 0; i < this.datacollection.labels.length; i++) {
